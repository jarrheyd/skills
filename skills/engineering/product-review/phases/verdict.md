...
- Write `review.json` in the review folder: `{ "feature", "target", "overall": "ALIGNED|NEEDS A CONVERSATION", "items": [{ "n", "expectation", "verdict", "note", "flow": "<flow name>", "actual": "<shot path>", "actuals": ["<shot path>", ...], "expected": "<figma export path>", "code": "<file:line>" }], "extras": ["..."], "bounds": ["code-only", "screen X unreachable"] }`. The report builder renders from this file.
- Every item whose screen was reachable carries a screenshot: `actual` for one, or `actuals` for the states a flow walks (filled, empty, error, after-success). Name the flow in `flow` so the reader ties the shot to the walk. Only a purely behavioral expectation (the screen is not the point) may lean on `code` alone. The builder renders a loud placeholder for any visual item missing a shot and prints a warning, so a flow without its screenshot is visible, never silently dropped.
